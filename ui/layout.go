...
type Layout struct {
	...
	msgIn           chan models.Message    // 消息刷新
	msgOut          chan models.MessageOut //  消息输出
	closeChan       chan int
	autoReply       chan int
	showUserList    []string
...
func NewLayout(myName, myID string, msgIn chan models.Message, msgOut chan models.MessageOut, closeChan, autoReply chan int, logger *log.Logger) *Layout {
	//用户列表框
